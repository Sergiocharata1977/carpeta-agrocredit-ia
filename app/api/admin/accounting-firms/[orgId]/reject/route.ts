...
export async function POST(
  ...
  try {
    ...
    await orgRef.update({ status: "rejected", rejectionReason: reason, updatedAt: now })
    ...
    await Promise.all(
      membersSnap.docs.map(async (memberDoc) => {
        ...
        await auth.setCustomUserClaims(uid, { ...existing, orgStatus: "rejected" })
      })
    )
    ...
    await writeAuditLog({
      ...
      action: "admin.accounting_firm_rejected",
      targetType: "organization",
      targetId: orgId,
      metadata: { legalName: orgSnap.data()?.legalName, reason },
    })
