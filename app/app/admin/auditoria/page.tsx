...
function formatTimestamp(value: unknown): string {
  ...
  if (typeof value === "string" && value.trim()) {
    ...
    return isNaN(d.getTime()) ? value : d.toLocaleString("es-AR")
  }
...
function readMetadataText(log: AuditLog, key: string, fallback: string): string {
        ...
        <section className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_21rem]">
          <div className="space-y-5">
            <div className="grid gap-4 lg:grid-cols-[1fr_1fr_1.4fr]">
              <article className="ag-card p-6">
                <div className="flex items-start justify-between">
                  <div>
                    <p className="text-[1.1rem] text-[var(--brand-ink)]">Registros cargados</p>
                    <p className="mt-6 text-[3rem] font-extrabold tracking-tight text-[var(--brand-ink)]">
                      {loading ? "..." : logs.length}
                    </p>
                    <p className="mt-2 text-lg text-[var(--brand-muted)]">
                      {loading ? "Cargando..." : logs.length === 0 ? "Sin actividad aún" : "Eventos de auditoría"}
                    </p>
                  </div>
            ...
            <section className="ag-panel overflow-hidden">
              ...
              {loading ? (
                <div className="px-6 py-12 text-center text-[var(--brand-muted)]">Cargando registros...</div>
              ) : logs.length === 0 ? (
                <div className="flex flex-col items-center gap-3 px-6 py-16 text-center">
                  <ShieldCheck className="h-10 w-10 text-[var(--brand-muted)]" />
                  <p className="text-lg font-semibold text-[var(--brand-ink)]">Sin actividad registrada aún</p>
                  <p className="text-sm text-[var(--brand-muted)]">
                    Los eventos de auditoría aparecerán aquí a medida que los usuarios interactúen con el sistema.
                  </p>
                </div>
              ) : (
                <>
