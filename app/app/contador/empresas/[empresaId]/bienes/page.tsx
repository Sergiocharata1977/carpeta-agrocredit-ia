...
export default function EmpresaBienesPage({ params }: PageProps) {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true)
    try {
      const [fetchedAssets, fetchedLiabilities] = await Promise.all([
        getAssetsForProducer(empresaId),
        getLiabilitiesForProducer(empresaId),
      ])
      setAssets(fetchedAssets)
      setLiabilities(fetchedLiabilities)
    } catch {
      toast.error("Error al cargar los bienes y pasivos")
    } finally {
      setLoading(false)
    }
  }, [empresaId])
  ...
  const getDialogTitle = () => {
    ...
    if (dialogMode.type === "real_estate") return dialogMode.asset ? "Editar inmueble" : "Agregar inmueble"
    if (dialogMode.type === "movable") return dialogMode.asset ? "Editar bien mueble" : "Agregar bien mueble"
    ...
    <div className="space-y-6">
      {loading ? (
        ...
        <Tabs defaultValue="inmuebles">
          <TabsList>
            <TabsTrigger value="inmuebles">Inmuebles ({realEstateAssets.length})</TabsTrigger>
            <TabsTrigger value="muebles">Bienes muebles ({movableAssets.length})</TabsTrigger>
            <TabsTrigger value="deudas">Deudas ({liabilities.length})</TabsTrigger>
            <TabsTrigger value="resumen">Resumen</TabsTrigger>
          </TabsList>

          <TabsContent value="inmuebles" className="space-y-4">
            <div className="flex justify-end">
              <Button onClick={() => setDialogMode({ type: "real_estate" })} size="sm">
                <Plus className="size-4 mr-1" /> Agregar inmueble
              </Button>
            </div>
            <AssetsTable
              assets={realEstateAssets}
              onEdit={(asset) => setDialogMode({ type: "real_estate", asset })}
          ...
          <TabsContent value="muebles" className="space-y-4">
            <div className="flex justify-end">
              <Button onClick={() => setDialogMode({ type: "movable" })} size="sm">
                <Plus className="size-4 mr-1" /> Agregar bien mueble
              </Button>
            </div>
            <AssetsTable
          ...
          <TabsContent value="deudas" className="space-y-4">
            <div className="flex justify-end">
              <Button onClick={() => setDialogMode({ type: "liability" })} size="sm">
                <Plus className="size-4 mr-1" /> Agregar deuda
              </Button>
            </div>
            <LiabilitiesTable
              liabilities={liabilities}
              onEdit={(liability) => setDialogMode({ type: "liability", liability })}
