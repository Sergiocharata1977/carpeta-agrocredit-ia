  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch("/api/auth/register", {
        ...
        body: JSON.stringify({ displayName, email, password, role: "accounting_firm" }),
      })
      ...
      <div className="w-full max-w-md space-y-6">
        ...
        <div className="rounded-2xl border border-[#dde4dc] bg-white p-8 shadow-sm space-y-6">
          <div className="flex items-center gap-3">
            <div className="flex size-10 items-center justify-center rounded-full bg-[#fff4e5] text-[#b56f2b]">
              <Calculator className="size-5" />
            </div>
            <div>
              <p className="text-xs font-semibold uppercase tracking-widest text-[#b56f2b]">Contador / Estudio contable</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-1.5">
              ...
              <Input id="displayName" name="displayName" placeholder="María López" required />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="email">Email</Label>
              <Input id="email" name="email" type="email" placeholder="maria@estudio.com" required />
            </div>
            <div className="space-y-1.5">
            ...
            <Button
              type="submit"
              disabled={loading}
              className="w-full rounded-full bg-[#b56f2b] text-white hover:bg-[#9e5f22]"
            >
        ...
        <p className="text-center text-xs text-[#59675f]">
          Una vez dentro, configurás el estudio y empezás a cargar clientes.
        </p>
      </div>
    </main>
  )
